...
| `run.py` | High-level stop/refine/resume policy informed by `src/matensemble/reactions/controller.py` at baseline commit; test scenario informed by `tests/test_reaction_controller.py` | RF-5 rewritten as a scheduler-independent synchronous ASE loop over ReactionFlow's existing components. No Pipeline, Chore, manager, Flux, monitor-service, or calculator-registry code was copied. |
